...
case class ReportWorkbookUpdater(reportWorkbook: ReportWorkbook, workbookUpdaterSemaphore: Semaphore) extends WorkbookUpdater {
    ...
    private lazy val sheetName2XSSFDrawing: Map[String, XSSFDrawing] = reportWorkbook.getSheets.map { sheet =>
    ...
    override def addLinkCellConceptKBIdComment(cell: XSSFCell, kbId: ID): UIO[Unit] = addStringComment(cell, s"{${kbId.toString}}")

    override def addLinkCellLinkKBIdComment(cell: XSSFCell, kbId: ID): UIO[Unit] = withSemaphore {
        UIO.succeed {
            Option(cell.getCellComment.getString.getString).foreach { conceptKBId =>
                cell.getCellComment.setString(s"$kbId $conceptKBId")
            }
        }
    }
    ...
    override def addSemicolons(cell: XSSFCell, n: Int): UIO[Unit] = addStringComment(cell, "; " * (n - 1)).when(n > 0)
    ...
    private def getXSSFCellStyle(color: XSSFColor): XSSFCellStyle = {
        val cellStyle = reportWorkbook.workbook.createCellStyle()
...
object ReportWorkbookUpdater {
    val layer: URLayer[Has[ReportWorkbook] with Has[Semaphore], Has[ReportWorkbookUpdater]] =
        (ReportWorkbookUpdater(_, _)).toLayer
    val workbookUpdaterLayer: URLayer[Has[ReportWorkbookUpdater], Has[WorkbookUpdater]] =
        Function1ToLayerSyntax[ReportWorkbookUpdater, WorkbookUpdater](identity).toLayer
}
